...
async fn setup_service() -> Result<TestService> {
    ...
    let mut cmd = Command::new("mysql-mcp");
    cmd.arg("stdio");
...
async fn test_table_operations() -> Result<()> {
    ...
    let create_result = service
        .call_tool(CallToolRequestParam {
            ...
            arguments: Some(object!({
                "conn_id": conn_id.as_str(),
                "query": "CREATE TABLE IF NOT EXISTS test_users (id BIGINT AUTO_INCREMENT PRIMARY KEY, name varchar(255) NOT NULL, email varchar(255) UNIQUE NOT NULL)"
            })),
        })
        .await?;
...
async fn test_type_operations() -> Result<()> {
    ...
    // Create enum type (MySQL doesn't support CREATE TYPE, use VARCHAR instead)
    let create_table_result = service
        .call_tool(CallToolRequestParam {
            name: "create_table".into(),
            arguments: Some(object!({
                "conn_id": conn_id.as_str(),
                "query": "CREATE TABLE test_users (id BIGINT AUTO_INCREMENT PRIMARY KEY, name TEXT NOT NULL, role VARCHAR(10) NOT NULL)"
            })),
        })
        .await?;
    assert!(!create_table_result.content.is_empty());

    // Insert data using VARCHAR for role
    let insert_result = service
        .call_tool(CallToolRequestParam {
            name: "insert".into(),
            arguments: Some(object!({
